-- Atomic, authenticated import path for uploaded TalentLens datasets.
-- SECURITY INVOKER preserves the caller's organization and RLS policies.
...
BEGIN
  ...
  ON CONFLICT (employee_code) DO UPDATE SET
    ...
    manager_changes = EXCLUDED.manager_changes,
  ...
  ON CONFLICT (candidate_code) DO UPDATE SET
    ...
    hiring_cost = EXCLUDED.hiring_cost,
  ...
  RETURN jsonb_build_object('success', true, 'imported', v_total_count);
END;
$$;
